...
const Products = () => {
  ...
  return (
    <main className="flex flex-col items-center p-4 bg-cyan-500 min-h-screen">
      <div id="filters" className="mb-4">
        <button onClick={() => filterProducts('all')} className="bg-cyan-500 text-white py-2 px-4 m-2 rounded hover:bg-cyan-500">Todos</button>
        {categoryList.map(category => (
          <button 
            key={category.name} 
            onClick={() => filterProducts(category.name)} 
            className="bg-cyan-500 text-white py-2 px-4 m-2 rounded hover:bg-cyan-500">
            {category.name}
          </button>
        ))}
      </div>
